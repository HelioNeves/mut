    ...
    def remove_duplications(self):
        ...
        for ix in range(len(self.data)):
            for yx in range(ix, len(self.data)):
                if  (ix != yx):
                    if jf.hamming_distance(self.data.summary[ix][0:200], self.data.summary[yx][0:200]) <= 20:
                        duplications.append(yx)
        
        duplications = list(set(duplications))
        self.data = self.data[~self.data.index.isin(duplications)].reset_index(drop=True)
...
# And stem sentences from dataset's summary
    def standardization(self):
        ...
        for ix in range(len(self.data)):
            ...
            self.data.at[ix, 'title'] = self.data.title[ix].lower().replace('\n', '').replace('\t', '').replace(',', ' ')
            self.data.at[ix, 'title'] = unidecode(self.data.at[ix, 'title'])
            
            # Company: formatation, accent
            self.data.at[ix, 'company'] = self.data.company[ix].lower().replace('\n', '').replace('\t', '').replace(',', ' ')
            self.data.at[ix, 'company'] = unidecode(self.data.at[ix, 'company'])
            
            # Summary: formatation, accent, number digits and stem
            self.data.at[ix, 'summary'] = self.data.summary[ix].lower().replace('\n', '').replace('\t', '').replace(',', ' ')
            self.data.at[ix, 'summary'] = unidecode(self.data.at[ix, 'summary'])
            self.data.at[ix, 'summary'] = "".join(filter(lambda x: not x.isdigit(), self.data.at[ix, 'summary']))
            self.data.at[ix, 'summary'] = self.stemming(self.tokenize(self.data.at[ix, 'summary']))
