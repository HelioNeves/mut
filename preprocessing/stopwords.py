...
# Load stopwords and toxic words
    def get(self, toxicwords_file):
        content = self.download('https://raw.githubusercontent.com/stopwords-iso/stopwords-pt/master/stopwords-pt.txt').split('\n')
        ...
        for word in range(len(content)):
            stopwords_ptbr.append(self.stemming(self.tokenize(unidecode(content[word]))))
